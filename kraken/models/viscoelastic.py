...
class StokesSolver:
    def __init__(self, msh, bc_func, material, dt):
        ...
        V_el = bufl.element("Lagrange", self.msh.basix_cell(), 2, shape=(self.msh.geometry.dim,), dtype=default_real_type)
        P_el = bufl.element("Lagrange", self.msh.basix_cell(), 1, dtype=default_real_type)

        self.V = fem.functionspace(self.msh, V_el)
        self.Q = fem.functionspace(self.msh, P_el)
    ...
    def setup(self,u,d,w):
        self.u = u
        self.d = d
        self.w = w
        

        du, dw = ufl.TrialFunction(self.V), ufl.TrialFunction(self.Q)
        dp = ufl.TrialFunction(self.Q)
        ...
        def σ(u,w):
            return mf.degraded_stress(ε(u-w),g,self.material.ν)
