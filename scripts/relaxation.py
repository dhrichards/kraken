...
model.params.l.value = l
model.params.dt.value = 60*60*72
...
for i in range(300):
    ...
    kr.iterators.fixed_point(model,min_its=min_its,tol=1e-4,solve_damage=solve_d,max_its=300)#tol=-1, max_its = 10)

    kr.utilities.write_xdmf(path + "/relax" + str(i) + ".xdmf",
                            msh, [model.u,model.d,es.free_energy_plus_dp(model.ε_e,model.params.ν.value),model.lap],["u","d","pp","lap"], t=i)
